...
-   **基础组件 (Component)**: 可在项目中随处复用的通用 UI 元素，如按钮 (`.ul-btn`)、表单 (`.ul-form`)、列表 (`.ul-list`) 等。它们是高度抽象、功能单一的构建块，旨在被其它组件组合或直接使用。例如，`.ul-card` 提供了一个最通用的卡片容器样式。
...
-   **工具/辅助类 (Utility)**: 提供原子化的、高复用性的 CSS 功能，用于微调样式。例如间距 (`.ul-m-1`, `.ul-p-1`)、浮动、文本对齐等。
